...
const trackSchema = {
    ...
    id: {
        type: "string",
        description: "The unique identifier of the track, same as the Spotify track ID included in the input"
    }
}
...
const generatePlaylists = async (req, res) => {
    ...
    const playlistString = req.body.playlists.map(playlist => playlist.playlist_name + ' - ' + playlist.playlist_description).join('\n');
    const trackString = req.body.tracks.map(track => track.name + ' - ' + track.artist + ` (id: ${track.id})`).join('\n');
    ...
    try {
        ...
        const completion = await openai.createChatCompletion({
            ...
            messages: [
                { 
                    role: 'system', 
                    content: "As a skillful assistant with music expertise, your job is to sort songs into playlists. The criteria for categorization includes mood, lyrics, tempo, and more. Each playlist in the 'playlists' array must contain: 'playlist_name', 'playlist_description', and 'tracks'. Each 'track' should have 'name', 'artist', and 'id'."
                },
                { 
                    role: 'user', 
            ...
            functions: [
                {
                    ...
                    description: "This function generates playlists from a given list of tracks. The playlists are created based on factors such as song mood, lyrics, tempo, and more. Each playlist has its own name and description.",
                    parameters: {
                        type: "object",
                        description: "This object represents a collection of generated playlists.",
                        properties: {
                            playlists: {
                                type: "array",
                                description: "This array contains objects representing individual playlists and their corresponding tracks.",
                                items: {
                                    playlist_name: {
                                        type: "string",
                                    ...
                                    tracks: {
                                        type: "array",
                                        description: "This array contains objects representing the tracks within the playlist.",
                                        items: trackSchema
                                    }
                                }
                            },
                            leftover_tracks: {
                                type: "array",
                                description: "This array contains objects representing the tracks that did not fit into any playlist.",
                                items: trackSchema
                            }
                        },
                        required: ["playlists", "leftover_tracks", "id", "playlist_name", "playlist_description", "name", "artist"]
                    }                
                }
            ],            
            function_call: "auto",
...
module.exports = { getNamesAndDescriptions, getNameAndDescriptionByKeyword, generatePlaylists }